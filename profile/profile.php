...
<head>
  ...
  .modal-input[type="file"]::file-selector-button {
    ...
    box-shadow: 0 4px 12px rgba(46, 89, 97, 0.25);
    margin-right: 12px;
  }

  .modal-input[type="file"]:hover::file-selector-button {
    transform: translateY(-2px);
    box-shadow: 0 6px 16px rgba(46, 89, 97, 0.35);
  }
  ...
  <div class="profilepage">
    ...
    <div class="main-content">
      ...
      <div class="left-column">
        ...
        <div class="profile-banner">
          <div class="banner-top" style="position:relative;">
            ...
            <label for="banner-upload" class="banner-edit-overlay">📷
              <?php echo empty($user['banner']) ? 'Ajouter une photo' : 'Changer la photo'; ?></label>
            <?php endif; ?>
          </div>
          <div class="avatar" style="position:relative;">
            <?php if (!empty($user['avatar'])): ?>
            <span class="avatar-placeholder"><img src="<?php echo htmlspecialchars($user['avatar']); ?>" alt=""
                id="avatar-img"></span>
            <?php else: ?>
            <span
              class="avatar-initials"><?php echo htmlspecialchars(strtoupper(substr($user['prenom'], 0, 1) . substr($user['nom'], 0, 1))); ?></span>
            <?php endif; ?>
            <?php if ($is_my_profile): ?>
          ...
          <form id="photo-upload-form" method="POST" action="upload_photo.php?id=<?php echo $profile_id; ?>"
          ...
          <div class="profile-info">
            ...
            <div id="banner-view">
              ...
              <p><?php echo htmlspecialchars($user['filliere'] ?? 'Filière'); ?> ·
                <?php echo htmlspecialchars($user['niveau'] ?? '2026'); ?> ·
                <?php echo htmlspecialchars($user['universite'] ?? $user['school'] ?? 'Université'); ?></p>
              <div class="contact">
                <a href="<?php echo htmlspecialchars($user['instagram'] ?? '#'); ?>"><img
                    src="../mainpage/image/svg/instagram-svgrepo-com.svg" alt=""></a>
                <a href="<?php echo htmlspecialchars($user['linkedin'] ?? '#'); ?>"><img
                    src="../mainpage/image/svg/linkedin-svgrepo-com.svg" alt=""></a>
                <a href="<?php echo htmlspecialchars($user['facebook'] ?? '#'); ?>"><img
                    src="../mainpage/image/svg/facebook-svgrepo-com.svg" alt=""></a>
              </div>
              <?php if ($is_my_profile): ?>
              <button type="button" class="left-edit-btn" onclick="toggleBannerEdit()" style="margin-top:10px;">✏️
                Edit</button>
              <?php endif; ?>
            </div>

            <!-- EDIT MODE (only for owner) — text fields only, photos are changed via overlays -->
            <form id="banner-edit" style="display:none" method="POST"
              action="update_banner.php?id=<?php echo $profile_id; ?>">
              ...
              <div class="edit-field">
                ...
                <input type="text" name="universite"
                  value="<?php echo htmlspecialchars($user['universite'] ?? $user['school'] ?? ''); ?>"
              ...
              <div class="edit-field">
                ...
                <input type="text" name="facebook" value="<?php echo htmlspecialchars($user['facebook'] ?? ''); ?>"
            ...
            <div class="fol-mes">
              ...
              <div class="follow">
                ...
                <form method="POST" action="unfollow.php" style="display:inline;">
                  ...
                  <button type="submit" class="follow-btn following">✓ Following
                    (<?php echo $followers_count; ?>)</button>
                </form>
                <?php else: ?>
              ...
              <div class="message">
                <button type="button" class="mes-but" onclick="toggleProfileChat()"><img
                    src="img/message-circle-dots-svgrepo-com.svg" alt="" class="message-img"></button>
                <div class="chat-tab" id="profile-chat-tab">
                  <div class="chat-header">
                    <span>💬 Message</span>
                    <button type="button" onclick="toggleProfileChat()"
                      style="background:none;border:none;font-size:1.2rem;cursor:pointer;color:#fff;">✕</button>
                  </div>
                  <div class="chat-body" id="profile-chat-body">
                    <p class="chat-placeholder">Chargement...</p>
                  </div>
                  <div class="chat-input-row">
                    <input type="text" id="profile-chat-input" placeholder="Écrire un message..." class="chat-input"
                      onkeydown="if(event.key==='Enter') sendProfileChatMessage()">
              ...
              <div class="profile-stats"
                style="display:flex; gap:1.5rem; margin-top:10px; font-size:0.9rem; color:#666;">
        ...
        <div class="info-block">
          ...
          <!-- VIEW MODE -->
          <div id="info-view">
            <div class="info-row"><span>Email</span><span><?php echo htmlspecialchars($user['email'] ?? '—'); ?></span>
            </div>
            <div class="info-row">
              <span>Téléphone</span><span><?php echo htmlspecialchars($user['phone'] ?? '—'); ?></span>
            </div>
            <div class="info-row">
              <span>Promotion</span><span><?php echo htmlspecialchars($user['promotion'] ?? '—'); ?></span>
            </div>
            <div class="info-row">
              <span>Spécialité</span><span><?php echo htmlspecialchars($user['specialite'] ?? '—'); ?></span>
            </div>
            <div class="info-row">
              <span>Niveau</span><span><?php echo htmlspecialchars($user['niveau'] ?? '—'); ?></span>
            </div>
          </div>
          ...
          <form id="info-edit" style="display:none" method="POST" action="update_info.php">
            ...
            <div class="edit-field">
              ...
              <input type="text" name="promotion" id="promotion"
                value="<?php echo htmlspecialchars($user['promotion'] ?? ''); ?>" placeholder="Promotion">
            </div>
            <div class="edit-field">
              <label for="specialite">Spécialité</label>
              <input type="text" name="specialite" id="specialite"
                value="<?php echo htmlspecialchars($user['specialite'] ?? ''); ?>" placeholder="Spécialité">
            </div>
            <div class="edit-field">
              <label for="niveau">Niveau</label>
              <input type="text" name="niveau" id="niveau"
                value="<?php echo htmlspecialchars($user['niveau'] ?? ''); ?>" placeholder="Niveau">
            </div>
            <div class="edit-actions">
        ...
        <div class="posts-block">
          ...
          <div class="post-card">
            ...
            <h4 style="margin:0 0 8px;font-size:1rem;font-weight:700;color:#1a2332;">
            ...
            <?php if ($is_my_profile): ?>
            <form method="POST" action="delete_post.php" style="margin-top:0.5rem;text-align:right;"
              onsubmit="return confirm('Delete this post?');">
              <input type="hidden" name="post_id" value="<?php echo $post['id']; ?>" />
              <button type="submit"
                style="background:none;border:none;color:#EF4444;cursor:pointer;font-size:0.8rem;padding:0;">🗑️
                Delete</button>
            </form>
            <?php endif; ?>
          </div>
      ...
      <div class="right-column">
        ...
        <div class="edit-panel" id="edit-panel" style="display:none">
          ...
          <div class="edit-input-wrap" id="edit-input-wrap" style="display:none">
            <form method="POST" action="add_section.php" class="edit-form">
              ...
              <textarea name="description" placeholder="Description (max 150 caractères)..." class="edit-text-input"
                maxlength="150" rows="2" required></textarea>
              <input type="text" name="date_value" placeholder="Date (ex: 2024, Jan 2024...)" class="edit-text-input"
                required>
        ...
        <div class="certificates">
          ...
          <div class="activity-item">
            <div class="activity-dot"></div>
            <div class="activity-content">
              <div class="activity-title"><?php echo htmlspecialchars($cert['title'] ?? $cert['content'] ?? ''); ?>
              </div>
    ...
    <div class="post-bubble-wrapper">
      ...
      <div class="bubbles-container" id="bubbles-container">
        ...
        <!-- Bubble 4 — Mentoring -->
        <button type="button" class="bubble bubble-mentoring" data-modal="modal-mentoring"
          aria-label="Nouveau post Mentoring">
          <img src="../pages/icons/mentoring.svg" alt="Mentoring">
          <span>Mentoring</span>
        </button>

        <!-- Bubble 5 — Bons Plans -->
        <button type="button" class="bubble bubble-bonplan" data-modal="modal-bonplan"
          aria-label="Nouveau post Bons Plans">
      ...
      <div class="modal-card">
        ...
        <div class="modal-body">
          <form action="create_post.php" method="POST" enctype="multipart/form-data" id="form-events">
            ...
            <!-- Description -->
            <textarea name="content" placeholder="Description de l'événement (programme, horaires...)"
              class="modal-textarea" required></textarea>
            <!-- Ville -->
            <input type="text" name="location" placeholder="Ville" class="modal-input" required>
            <!-- Type d'événement -->
            <input type="text" name="type_event" placeholder="Type d'événement (ex: Conférence, Concert, Soirée...)"
              class="modal-input">
            <!-- Date -->
            ...
            <div class="modal-row modal-row-tarif">
              <input type="number" name="tarif" id="tarif-events" placeholder="Tarif en €" class="modal-input" min="0"
                step="0.01">
      ...
      <div class="modal-card">
        ...
        <div class="modal-body">
          <form action="create_post.php" method="POST" enctype="multipart/form-data">
            ...
            <!-- Titre -->
            <input type="text" name="title" placeholder="Intitulé du poste (ex: Développeur Web...)" class="modal-input"
              required>
            <!-- Description -->
            <textarea name="content" placeholder="Description du stage (missions, profil recherché...)"
              class="modal-textarea" required></textarea>
            <!-- Domaine -->
            <input type="text" name="domaine" placeholder="Domaine (ex: Informatique, Marketing, Finance...)"
              class="modal-input">
            <!-- Ville -->
            ...
            <!-- Niveau d'étude -->
            <input type="text" name="niveau_etude" placeholder="Niveau d'étude recherché (ex: Bac+3, Master...)"
              class="modal-input">
      ...
      <div class="modal-card">
        ...
        <div class="modal-body">
          <form action="create_post.php" method="POST">
            ...
            <!-- Titre -->
            <input type="text" name="title" placeholder="Sujet de mentorat (ex: Aide en maths, CV review...)"
              class="modal-input" required>
            <!-- Description -->
            <textarea name="content" placeholder="Décrivez ce que vous proposez ou recherchez..." class="modal-textarea"
              required></textarea>
            <!-- Matière -->
            <input type="text" name="matiere" placeholder="Matière (ex: Mathématiques, Programmation...)"
              class="modal-input">
            <!-- Niveau -->
            <input type="text" name="niveau_mentoring" placeholder="Niveau (ex: Licence, Master, Débutant...)"
              class="modal-input">
            <!-- Langue -->
            ...
            <div class="modal-row modal-row-tarif">
              <input type="number" name="prix_mentoring" id="prix-mentor" placeholder="Prix en €/heure"
                class="modal-input" min="0" step="0.01">
              <label class="modal-check-label">
                <input type="checkbox" name="is_mentor" value="mentor" id="check-mentor"
                  onchange="toggleMentorPrice(this)">
      ...
      <div class="modal-card">
        ...
        <div class="modal-body">
          <form action="create_post.php" method="POST" enctype="multipart/form-data">
            ...
            <!-- Description -->
            <textarea name="content" placeholder="Description du produit (détails, défauts, raison de vente...)"
              class="modal-textarea" required></textarea>
            <!-- État -->
      ...
      <div class="modal-card">
        ...
        <div class="modal-body">
          <form action="create_post.php" method="POST" enctype="multipart/form-data">
            ...
            <input type="file" name="image" accept="image/*" class="modal-input" required>
            <button type="submit" class="modal-submit" style="background: linear-gradient(90deg, #1a3a40, #2E5961);">📤
              Publier</button>
          </form>
        </div>
      </div>
    ...
    (function() {
        ...
        try {
          ...
          if (!convData.error && convData.conversations) {
            ...
            if (conv) {
              ...
              const msgRes = await fetch('../pages/api/get_messages.php?conversation_id=' + conv.conversation_id + (
                profileChatLastId > 0 ? '&after=' + profileChatLastId : ''));
          ...
          // No conversation yet
          document.getElementById('profile-chat-body').innerHTML =
            '<p class="chat-placeholder">Commencez la conversation...</p>';
        } catch (e) {
          console.error(e);
        }
      }
      ...
      function renderProfileChatMessages(messages) {
        ...
        messages.forEach(msg => {
          ...
          html += '<div style="display:flex;flex-direction:column;gap:2px;margin-bottom:8px;' + (isMe ?
              'align-items:flex-end;' : 'align-items:flex-start;') + '">' +
            '<div style="' + bubbleStyle +
            'padding:8px 12px;font-size:0.88rem;max-width:85%;word-break:break-word;">' + escapeHtml(msg
              .content) + '</div>' +
            '<span style="font-size:0.7rem;color:#9ca3af;">' + timeAgoShort(msg.created_at) + (isMe ? (msg
              .is_read ? ' ✓✓' : ' ✓') : '') + '</span>' +
            '</div>';
        });
        body.innerHTML = html;
      ...
      window.sendProfileChatMessage = async function() {
        ...
        try {
          const res = await fetch('../pages/api/send_message.php', {
            method: 'POST',
            body: formData
          });
